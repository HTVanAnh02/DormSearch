<template>
    <div style="margin: 1.5%;">
      <v-row>
        ...
        <v-col cols="7" class="text-right" lg="9" sm="8" md="8">
          <v-btn @click="addRole()" color="#0F60FF" prepend-icon="mdi mdi-plus" class="text-capitalize">
              ...
              <tbody>
                <tr v-for="i in users" :key="i">
                  ...
                  <td class="text-center">
                    <v-btn density="compact" variant="text" @click="updateRoleById(i.id)" style="max-width: 24px;">
            ...
            <v-row class="ma-2">
              <v-col cols="8" sm="8" md="8" lg="8">
                <v-row>
                  ...
                  <span class="mt-5 opacity">of {{ TotalRole }}</span>
                </v-row>
              </v-col>
    ...
    <Confirmations v-model="isDialogDelete" @close="close()" :idDelete="idDelete" @delete="deleteUserById" />
  </template>
  <script setup>
  ...
  import { roleApi } from '../Roles/Services/role.api';
  ...
  const { fetchRoles, users, query, searchRoles } = useRole()
  const search = ref(null)
  const TotalRole = ref(null)
  ...
  const loadData = async () => {
    ...
    TotalRole.value = res.totalItems
    return
  }
  const addRole = () => {
  ...
  const deleteUserById = async (id) => {
    const data = await roleApi._delete(id)
    if (data.success) {
      loadData()
