            ...
            <v-card style="border-radius: 12px !important;">
                ...
                <v-container class="mt-10" style="background-color: #F7F8FA">
                    <div style="display: block; margin-top: 8px;">
                        ...
                        <v-text-field class="mt-1" v-model="areaname" placeholder="Nhập tên khu vực"
                            :error-messages="areanameError" style="background-color: white;" density="compact"
                            single-line hide-details variant="outlined"></v-text-field>
                        <span style="color:red">{{ areanameError }}</span>
                    </div>
                </v-container>
...
const { value: areaname, errorMessage: areanameError } = useField(
    'areaname',
    yup
        .string()
...
const submit = handleSubmit(async () => {
    try {
        ...
        formData.append('areaname', areaname.value);
        if (props.itemEdit == null) {
