<template>
      ...
      <v-col cols="12">
        <v-card class="mb-4" style="border-radius: 16px; border: 1px;">
          <v-table density="compact">
            ...
            <tbody>
              <tr v-if="roles.length > 0" v-for="(item, index) in roles" :key="index">
          ...
          <v-row class="ma-2 ">
            <v-col cols="8" sm="8" md="8" lg="8">
              <v-row>
                ...
                <span class="mt-5 opacity">of {{ TotalRoles }}</span>
              </v-row>
            </v-col>
  ...
  <RolesDialog v-model="isShowDialog" :itemEdit="idEdit" @close="close()" @loadData="loadData()" />
  <Confirmations v-model="isDialogDelete" @close="close()" :idDelete="idDelete" @delete="deleteRole" />
</template>
<script setup>
...
const loadData = async () => {
  ...
  if (res.data) {
    ...
    TotalRoles.value = res.totalItems
    return
  }
  roles.value = []
}
